...
-    if (!formData.phone || !phoneRegex.test(formData.phone.replace(/\D/g, ''))) {
   ...
   const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!rateLimitCheck(clientId, 3, 300000)) { // 3 attempts per 5 minutes
      alert('Muitas tentativas. Aguarde alguns minutos antes de tentar novamente.');
      return;
    }
...
+              <h3 className="text-xl lg:text-2xl font-bold text-gray-900">Agendar Consulta</h3>
             </div>
...
+              <h3 className="text-xl lg:text-2xl font-bold text-gray-900 mb-6">Informações de Contato</h3>
...
import { sanitizeInput, validatePhone, validateName, rateLimitCheck } from '../utils/security';
